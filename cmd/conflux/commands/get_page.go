...
You must provide a space key and either a numeric page ID or a page title.`,
...
func runGetPage(cmd *cobra.Command, args []string) error {
	if getPageSpace == "" {
		return fmt.Errorf("space flag is required for get-page command")
	}
	if getPageIDOrTitle == "" {
...
func init() {
	...
	getPageCmd.Flags().StringVarP(&getPageSpace, "space", "s", "", "Confluence space key (required)")
	...
	if err := getPageCmd.MarkFlagRequired("space"); err != nil {
